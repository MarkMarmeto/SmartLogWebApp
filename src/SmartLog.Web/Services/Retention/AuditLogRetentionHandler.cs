...
public class AuditLogRetentionHandler : IEntityRetentionHandler
{
    private static readonly SemaphoreSlim _lock = new(1, 1);

    private readonly ApplicationDbContext _db;
    private readonly ILogger<AuditLogRetentionHandler> _logger;
    private readonly IArchiveService? _archiveService;

    public string EntityName => "AuditLog";

    public AuditLogRetentionHandler(
        ApplicationDbContext db,
        ILogger<AuditLogRetentionHandler> logger,
    ...
    public async Task<RetentionPreview> PreviewAsync(CancellationToken ct = default)
    {
        ...
        var eligible = _db.AuditLogs.Where(a => a.Timestamp < cutoff && !a.LegalHold);
        var count = await eligible.CountAsync(ct);
        var oldest = count > 0 ? await eligible.MinAsync(a => (DateTime?)a.Timestamp, ct) : null;
        var newest = count > 0 ? await eligible.MaxAsync(a => (DateTime?)a.Timestamp, ct) : null;
    ...
    public async Task<RetentionResult> ExecuteAsync(bool dryRun, CancellationToken ct = default, string runMode = "Manual")
    {
        ...
        try
        {
            ...
            try
            {
                if (dryRun)
                {
                    totalAffected = await _db.AuditLogs
                        .Where(a => a.Timestamp < cutoff && !a.LegalHold)
                        .CountAsync(ct);
                }
                else
                {
