...
public class SmsLogRetentionHandler : IEntityRetentionHandler
{
    private static readonly SemaphoreSlim _lock = new(1, 1);

    private readonly ApplicationDbContext _db;
    private readonly ILogger<SmsLogRetentionHandler> _logger;
    private readonly IArchiveService? _archiveService;

    public string EntityName => "SmsLog";

    public SmsLogRetentionHandler(
        ApplicationDbContext db,
        ILogger<SmsLogRetentionHandler> logger,
    ...
    public async Task<RetentionPreview> PreviewAsync(CancellationToken ct = default)
    {
        ...
        var eligible = _db.SmsLogs.Where(l => l.CreatedAt < cutoff);
        var count = await eligible.CountAsync(ct);
        var oldest = count > 0 ? await eligible.MinAsync(l => (DateTime?)l.CreatedAt, ct) : null;
        var newest = count > 0 ? await eligible.MaxAsync(l => (DateTime?)l.CreatedAt, ct) : null;
    ...
    public async Task<RetentionResult> ExecuteAsync(bool dryRun, CancellationToken ct = default, string runMode = "Manual")
    {
        ...
        try
        {
            ...
            try
            {
                if (dryRun)
                {
                    totalAffected = await _db.SmsLogs
                    ...
                    do
                    {
                        ...
                        if (batchRows > 0)
                        {
                            if (policy.ArchiveEnabled)
                            {
                                ...
                                {
                                    _logger.LogError(
                                        "{Entity} archive failed: {Error}", EntityName, archiveResult.ErrorMessage);
