...
public class SmsQueueRetentionHandler : IEntityRetentionHandler
{
    ...
    private static readonly SmsStatus[] EligibleStatuses = [SmsStatus.Sent, SmsStatus.Failed, SmsStatus.Cancelled];
    ...
    public SmsQueueRetentionHandler(ApplicationDbContext db, ILogger<SmsQueueRetentionHandler> logger)
    {
        _db = db;
        _logger = logger;
    }
    ...
    public async Task<RetentionPreview> PreviewAsync(CancellationToken ct = default)
    {
        ...
        var eligible = _db.SmsQueues.Where(q => EligibleStatuses.Contains(q.Status) && q.CreatedAt < cutoff);
        var count = await eligible.CountAsync(ct);
        var oldest = count > 0 ? await eligible.MinAsync(q => (DateTime?)q.CreatedAt, ct) : null;
        var newest = count > 0 ? await eligible.MaxAsync(q => (DateTime?)q.CreatedAt, ct) : null;
    ...
    public async Task<RetentionResult> ExecuteAsync(bool dryRun, CancellationToken ct = default, string runMode = "Manual")
    {
        ...
        try
        {
            ...
            try
            {
                if (dryRun)
                {
                    totalAffected = await _db.SmsQueues
                        .Where(q => EligibleStatuses.Contains(q.Status) && q.CreatedAt < cutoff)
                        .CountAsync(ct);
                }
                else
                {
                    ...
                    do
                    {
                        ...
                        if (batchRows > 0)
                        {
                            _db.SmsQueues.RemoveRange(batch);
