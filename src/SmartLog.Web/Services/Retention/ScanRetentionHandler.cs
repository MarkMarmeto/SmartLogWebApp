...
public class ScanRetentionHandler : IEntityRetentionHandler
{
    private static readonly SemaphoreSlim _lock = new(1, 1);

    private readonly ApplicationDbContext _db;
    private readonly ILogger<ScanRetentionHandler> _logger;
    private readonly IArchiveService? _archiveService;

    public string EntityName => "Scan";

    public ScanRetentionHandler(
        ApplicationDbContext db,
        ILogger<ScanRetentionHandler> logger,
    ...
    public async Task<RetentionPreview> PreviewAsync(CancellationToken ct = default)
    {
        ...
        var eligible = _db.Scans.Where(s => s.ScannedAt < cutoff);
        var count = await eligible.CountAsync(ct);
        var oldest = count > 0 ? await eligible.MinAsync(s => (DateTime?)s.ScannedAt, ct) : null;
        var newest = count > 0 ? await eligible.MaxAsync(s => (DateTime?)s.ScannedAt, ct) : null;
    ...
    public async Task<RetentionResult> ExecuteAsync(bool dryRun, CancellationToken ct = default, string runMode = "Manual")
    {
        ...
        try
        {
            ...
            try
            {
                if (dryRun)
                {
                    totalAffected = await _db.Scans
                        .Where(s => s.ScannedAt < cutoff)
                        .CountAsync(ct);
                }
                else
                {
